...
public:
    ...
    void AllocateAt(int list_index)
    {
        if (list_index >= list_heads.size()) {
            ...
            list_heads.insertAt(-1, list_index);
           ...
           assert(list_heads[list_index] == Null);
    ...
    bool Remove(int list_index, int val)
    {
        ...
        int iEnd = block_ptr + std::min(N,block);
    ...
    {
        assert(list_heads[to_index] == Null);
        assert(list_heads[from_index] != Null);
...
protected:
    ...
    int allocate_block()
    {
        ...
        block_store.insertAt(-1, nsize + BLOCK_LIST_OFFSET);
        block_store[nsize] = 0;
        allocated_count++;
        return nsize;
    }
