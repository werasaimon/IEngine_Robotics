...
unsigned int Simplex::getSimplex(Vector3* suppPointsA,
                                 Vector3* suppPointsB,
...
void Simplex::computeClosestPointsOfAandB(Vector3& pA, Vector3& pB) const
{
    ...
    assert(deltaX > 0.0);
    scalar factor = scalar(1.0) / deltaX;
    pA *= factor;
    pB *= factor;
}
...
{
    scalar minDistSquare = 0.00001;//DECIMAL_LARGEST;
    Bits bit;
...
Vector3 Simplex::computeClosestPointForSubset(Bits subset)
{
    ...
    scalar deltaX = 0.0;            // deltaX = sum of all det[subset][i]
    int i;
    Bits bit;
