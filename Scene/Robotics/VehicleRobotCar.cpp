...
RobotSensors::RobotSensors(ISensorEncoder *_a,
                           ISensorEncoder *_b,
                           ISensorEncoder *_c,
                           ISensorEncoder *_d,
    ...
    enc_d(_d)
{
    mOrientationSensor = new ISensorOrientation2(_PhysicsBase);
    pos_motorA = 0;
    pos_motorB = 0;
    pos_motorC = 0;
    pos_motorD = 0;
...
//    }
    ...
    if(direction.Length() > /*mDispatcherAttribute.minimal*/ 1 )
    {
        ...
        if(IMath::IAbs(IMath::Cross(-Vector3::X ,dir).LengthSquare()) < 0.001f )
        {
